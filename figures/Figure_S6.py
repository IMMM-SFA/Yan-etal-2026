...
vars_info = [
    ('GPP95', 'Growing Season (GS) GPP 95th', r'(gC m$^{-2}$ day$^{-1}$)'),
    ('iWUE_mean', 'Inherent Water-Use Efficiency', r'(gC kPa kg$^{-1}$)'),
...
fig, axes = plt.subplots(3, 1, figsize=(7, 10), layout='compressed')
...
for ax, (col, label, unit), panel in zip(axes, vars_info, panel_labels):
    ...
    # error bar lengths relative to the median (bar height)
...
plt.savefig('vegetation_biophysics_by_region.png', dpi=150)
print('saved')